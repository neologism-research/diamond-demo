...
import {DiamondOwnership} from "./DiamondOwnership.sol";

abstract contract DiamondCoreCut is DiamondOwnership {
    enum FacetCutAction {
        Add,
        Replace,
        Remove
    ...
    function addFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        require(_facetAddress != address(0), "Add facet cannot be zero address");
    ...
    function removeFunctions(address _facetAddress, bytes4[] memory _functionSelectors) internal {
        ...
        require(_facetAddress == address(0), "Remove facet address must be zero address");
        for (uint256 selectorIndex; selectorIndex < _functionSelectors.length; selectorIndex++) {
            bytes4 selector = _functionSelectors[selectorIndex];
            address oldFacetAddress = ds.selectorToFacetAndPosition[selector].facetAddress;
            removeFunction(oldFacetAddress, selector);
    ...
    function removeFunction(address _facetAddress, bytes4 _selector) internal {
        DiamondStorage storage ds = getStorage();
        require(_facetAddress != address(0), "Cannot remove function that doesn't exist");
        // an immutable function is a function defined directly in a diamond
        require(_facetAddress != address(this), "Cannot remove immutable function");
    ...
    function initializeDiamondCut(address _init, bytes memory _calldata) internal {
        if (_init == address(0)) {
            ...
            require(_calldata.length > 0, "_calldata is empty but _init is not zero address");
